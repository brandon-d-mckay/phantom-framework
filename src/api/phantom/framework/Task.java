...
import phantom.util.Lists;

public interface Task
{
	static ProcedureLambda lambda(ProcedureLambda lambda) { return lambda; }
	static ProcedureLambda procedure(ProcedureLambda lambda) { return lambda; }
	static <I> ConsumerLambda<I> lambda(ConsumerLambda<I> lambda) { return lambda; }
	static <I> ConsumerLambda<I> consumer(ConsumerLambda<I> lambda) { return lambda; }
	static <O> ProducerLambda<O> lambda(ProducerLambda<O> lambda) { return lambda; }
	static <O> ProducerLambda<O> producer(ProducerLambda<O> lambda) { return lambda; }
	static <I, O> FunctionLambda<I, O> lambda(FunctionLambda<I, O> lambda) { return lambda; }
	static <I, O> FunctionLambda<I, O> function(FunctionLambda<I, O> lambda) { return lambda; }
	...
	static void start(byte metadata, ProcedureLambda... lambdas) { Phantom.dispatch(Lists.map(lambdas, b -> b.unmask(metadata).construct().createNewJob(), Job[]::new)); }
	static void start(ProcedureLambda lambda) { lambda.start(); }
	static void start(ProcedureLambda... lambdas) { start((ProcedureBuilder[]) lambdas); }
	static void start(ProcedureBuilder... builders) { Phantom.dispatch(Lists.map(builders, b -> b.unmask().construct().createNewJob(), Job[]::new)); }
	static <I> void start(byte metadata, I input, ConsumerLambda<I> lambda) { lambda.unmask(metadata).start(input); }
	@SafeVarargs static <I> void start(byte metadata, I input, ConsumerLambda<I>... lambdas) { Phantom.dispatch(Lists.map(lambdas, b -> b.unmask(metadata).construct().createNewJob(input), Job[]::new)); }
	static <I> void start(I input, ConsumerLambda<I> lambda) { lambda.start(input); }
	@SafeVarargs static <I> void start(I input, ConsumerLambda<I>... lambdas) { start(input, (ConsumerBuilder<I>[]) lambdas); }
	@SafeVarargs static <I> void start(I input, ConsumerBuilder<I>... builders) { Phantom.dispatch(Lists.map(builders, b -> b.unmask().construct().createNewJob(input), Job[]::new)); }
}
